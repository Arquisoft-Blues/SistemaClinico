...
from sistema_clinico.apps.eventos.models import EventoClinico
from sistema_clinico.apps.examenes.models import Examen
import json
from django.views.decorators.csrf import csrf_exempt

def index(request):
    return JsonResponse({"message": "Reportes API funcionando correctamente."})
...
def obtener_reporte_completo(request, paciente_id):
    ...
    eventos = EventoClinico.objects.filter(paciente_id=paciente.id)  
...
def crear_paciente(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse({"id": paciente.id, "mensaje": "Paciente creado correctamente"}, status=201)
